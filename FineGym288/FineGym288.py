...
def get_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Command-Line tool for downloading FineGym288 dataset")
    parser.add_argument(
...
def get_csv() -> pd.DataFrame:
    ...
    for split in ["train", "val"]:
        ...
        with (Path(__file__).resolve().parent / f"{split}.txt").open(mode="r") as f:
        ...
        for line in txt_info:
            ...
            if name not in lines.keys():
                lines[name] = {"name": name,
                               "resolution": "0x0", "fps": 0, "yt_id": name}
...
def main(args: argparse.Namespace):
    ...
    执行下载FineGym288数据集的主要功能，包括设置ffmpeg路径、测试代理、下载视频和将视频下载结果写入文件。
