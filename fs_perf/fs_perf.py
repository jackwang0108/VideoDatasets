...
def get_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Command-Line tool for downloading fs_perf dataset")
    parser.add_argument(
...
def main(args: argparse.Namespace):
    ...
    执行fs_perf数据集下载程序的主要功能，包括设置ffmpeg路径、测试代理、下载视频和将结果写入文件。
...
if __name__ == "__main__":
    if not (Path(__file__).resolve().parent / "videos.csv").exists():
        get_csv()
    main(get_args())
